...
from basics import l1Distance, euclideanDistance
from inputManipulation import assignManipulationSimple, assignManipulation
...
class searchAstar:
    ...
    def reportCurrentValue(self,image):
        vs = []
        if len(self.consideringRegion[0]) == 2: 
            for (i,j) in self.consideringRegion: 
                vs.append(image[i][j])
        print "current values for the region: %s"%(vs)
        

    def defineConsideringRegion(self,rg):
        #rg = random.sample(rg,numEltInGroup)
        ...
        for i in rg: 
            span[i] = 0.0
            numSpan[i] = 1.0
        return assignManipulationSimple(self.images[(-1,-1)],span,numSpan)
        
    def initialiseActions(self): 
        i = 0
        allDims = copy.deepcopy(self.consideringRegion)
        while allDims != []: 
            indices = random.sample(allDims,numEltInGroup)
            span = {}
            numSpan = {}
            for j in indices: 
                span[j] = 1.0 / float(cuttingNum)
                numSpan[j] = 1.0
            self.actions[i] = (span,numSpan,numEltInGroup)
            allDims = list(set(allDims) - set(indices))
            i += 1
    ...
    def addImages(self,model,parentIndex,ims,manipulatedDims,stepsUpToNow):
        ...
        for (image,conf) in ims: 
            sys.stdout.write('.')
            for (span,numSpan,nn) in self.actions.values():
    ...
    def computeCost(self,model,image0):
        ...
        (dist, scale) = costForDijkstra
        if dist == "euclidean": 
            return euclideanDistance(image0,originalImage) + scale * (1-c)
        elif dist == "l1": 
            return l1Distance(image0,originalImage) + scale * (1-c)
